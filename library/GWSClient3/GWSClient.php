  ...
  */
  ...
  class GWSClient {
      ...
      public function send_request(CVO_WSRequest $request, $token, $aes){
		if ($this->enable_log) {
			// log all request into log file
			$datepostfix = date("Ymd"); 
			$logfile_address = dirname(__FILE__)."/logs/$datepostfix.php";
			$time = date("Y-m-d H:i:s e");
			if(!file_exists($logfile_address)){
				if(!file_exists(dirname($logfile_address))){
					mkdir(dirname($logfile_address));
				}
				error_log("<?php exit(); ?>",3,$logfile_address);
			}
			if ($request->method == 'register' || $request->method == 'auth_by_username' || $request->method == 'auth_by_uid' ) {
				// for these special API we only log the first parameter
				// this is so that we never store the user md5 password on log file!!!
				$params_log = $request->params[0].",...";
			} else {
				$params_log = implode(',',$request->params);
			}
			error_log("\r\n\r\n[$time] [".$_SERVER['REMOTE_ADDR']."] ".$_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI']
					."\r\n".$request->module.".".$request->method."(".$params_log.")",3,$logfile_address);      
			// end of log
		}
		
		  $fp=fopen(PRIVATEKEY_FILE, 'r');
          ...
          openssl_private_encrypt($token, $crypttext, $res);
